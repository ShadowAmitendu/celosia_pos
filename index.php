...
$stmt = $pdo->query("SELECT * FROM popular_products ORDER BY total_sold DESC LIMIT 5");
...
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
    <?php
    // Get top 4 popular products
    $topProducts = array_slice($popularProducts, 0, 4);
    foreach ($topProducts as $product):
        ?>
        <div class="bg-white rounded-2xl shadow-lg p-6 border-t-4 border-pastel-green hover:shadow-xl transition-shadow">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-gray-500 text-sm font-medium mb-1"><?php echo htmlspecialchars($product['product_name']); ?></p>
                    <h3 class="text-2xl font-bold text-gray-800"><?php echo $product['total_sold']; ?> sold</h3>
                    <p class="text-gray-600 text-sm mt-1">₹<?php echo number_format($product['total_revenue'], 2); ?>
                        revenue</p>
                </div>
                <div class="bg-pastel-green rounded-full p-4">
                    <svg class="w-8 h-8 text-gray-700"
                         fill="none"
                         stroke="currentColor"
                         viewBox="0 0 24 24">
    ...
    <?php else: ?>
        <div class="overflow-x-auto">
            <table class="w-full">
                <thead>
                <tr class="border-b border-gray-200">
                    <th class="text-left py-3 px-4 text-gray-600 font-semibold text-sm">Product</th>
                    <th class="text-left py-3 px-4 text-gray-600 font-semibold text-sm">Category</th>
                    <th class="text-right py-3 px-4 text-gray-600 font-semibold text-sm">Price</th>
                    <th class="text-right py-3 px-4 text-gray-600 font-semibold text-sm">Units Sold</th>
                    <th class="text-right py-3 px-4 text-gray-600 font-semibold text-sm">Revenue</th>
                </tr>
                </thead>
                <tbody>
                <?php foreach ($popularProducts as $product): ?>
                    <tr class="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                        <td class="py-3 px-4 font-medium text-gray-800"><?php echo htmlspecialchars($product['product_name']); ?></td>
                        <td class="py-3 px-4 text-gray-600"><?php echo htmlspecialchars($product['category']); ?></td>
                        <td class="py-3 px-4 text-right text-gray-800">
                            ₹<?php echo number_format($product['price'], 2); ?></td>
                        <td class="py-3 px-4 text-right font-semibold text-gray-800"><?php echo $product['total_sold']; ?></td>
                        <td class="py-3 px-4 text-right font-semibold text-gray-800">
                            ₹<?php echo number_format($product['total_revenue'], 2); ?></td>
                    </tr>
                <?php endforeach; ?>
                </tbody>
            </table>
